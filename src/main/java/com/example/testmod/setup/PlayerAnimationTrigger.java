...
public class PlayerAnimationTrigger {
    ...
    public static void onChatReceived(ClientChatReceivedEvent event) {
        ...
        if (event.getMessage().contains(Component.literal("waving"))) {
            ...
            var animation = (ModifierLayer<IAnimation>) PlayerAnimationAccess.getPlayerAssociatedData((AbstractClientPlayer) player).get(new ResourceLocation(TestMod.MODID, "animation"));
            if (animation != null) {
                //You can set an animation from anywhere ON THE CLIENT
                //Do not attempt to do this on a server, that will only fail

                animation.setAnimation(new KeyframeAnimationPlayer(PlayerAnimationRegistry.getAnimation(new ResourceLocation(TestMod.MODID, "waving"))));
