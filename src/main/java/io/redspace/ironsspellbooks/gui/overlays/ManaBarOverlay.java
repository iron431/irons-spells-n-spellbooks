...
public class ManaBarOverlay implements LayeredDraw.Layer {
    ...
    public void render(GuiGraphics guiHelper, DeltaTracker deltaTracker) {
        ...
        int mana = (int) ActualMagicData.get(player).getMana();
        int barX, barY;
        //TODO: cache these?
    ...
    public static boolean shouldShowManaBar(Player player) {
        ...
        return !player.isSpectator() && display != Display.Never &&
                (display == Display.Always || player.isHolding(itemStack -> itemStack.getItem() instanceof CastingItem || (ISpellContainer.isSpellContainer(itemStack) && !ISpellContainer.get(itemStack).mustEquip())) || ActualMagicData.get(player).getMana() < player.getAttributeValue(MAX_MANA));
