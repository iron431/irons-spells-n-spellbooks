...
public class FallingBlockParticle extends TextureSheetParticle {
    ...
    FallingBlockParticle(ClientLevel pLevel, double pX, double pY, double pZ, double xd, double yd, double zd, BlockParticleOption options) {
        ...
        if (this.blockState.isAir() || onGround || age > lifetime) {
            if (onGround) {
                if (particlesOnImpact) {
                    double speed = Math.sqrt(xd * xd + yd * yd + zd * zd) + 0.1;
                    for (int i = 0; i < 25; i++) {
                        Vec3 random = Utils.getRandomVec3(1).multiply(1, 0.25, 1).normalize().scale(speed + 0.1);
    ...
    public static class Provider implements ParticleProvider<BlockParticleOption> {
        ...
        public Particle createParticle(@NotNull BlockParticleOption options, @NotNull ClientLevel pLevel, double pX, double pY, double pZ, double pXSpeed, double pYSpeed, double pZSpeed) {
