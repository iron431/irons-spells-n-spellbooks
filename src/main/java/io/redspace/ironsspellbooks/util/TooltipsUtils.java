...
//                Component.literal("" + spell.getLevel())).withStyle(spellType.getRarity(spell.getLevel()).getDisplayName().getStyle());
        var title = Component.translatable("tooltip.irons_spellbooks.selected_spell",
                ...
                Component.literal("" + spell.getLevel(player))).withStyle(spellType.getSchoolType().getDisplayName().getStyle());
    ...
    public static List<Component> formatScrollTooltip(ItemStack stack) {
        ...
        if (spellType == SpellType.NONE_SPELL)
            return List.of();
        var title = Component.translatable("tooltip.irons_spellbooks.level", spell.getLevel(player)).append(" ").append(Component.translatable("tooltip.irons_spellbooks.rarity", spell.getRarity().getDisplayName().getString())).withStyle(spell.getRarity().getDisplayName().getStyle());
        var uniqueInfo = spell.getUniqueInfo(player);
