...
public class LocateArcaneDebris {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> pDispatcher) {
        pDispatcher.register(Commands.literal("locateArcaneDebris").requires((p_138819_) -> {
        ...
        for (int i = yFrom; i < yTo; i++) {
            for (int j = xFrom; j < xTo; j++) {
                for (int k = zFrom; k < zTo; k++) {
                    ...
                    if (blockstate.is(BlockRegistry.ARCANE_DEBRIS.get())) {
